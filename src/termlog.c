...
static int init_termlog(traft_log_term_log *log, const char *entries_file) {
...
int traft_termlog_open(traft_log_term_log *log, const char *basedir, uint64_t term_id) {
...
int traft_termlog_create(traft_log_term_log *log, const char *basedir, uint64_t term_id, uint32_t num_entries) {
...
static int append_entry_internal(traft_termlog *log, append_entries_req *header, traft_buff *entry) {
  uint32_t prev_idx = log->header->local_committed_idx;
  off_t    next_entry_pos = log->entries[prev_idx].pos + log->entries[prev_idx].len;
  
  // Assert we can add this entry
  if (header->prev_idx != prev_idx) {
    return -1;
  }
  if (header->prev_idx + 1 == header->max_entries) {
    return -1;
  }
  // Execute write
  ...
  log->entries[header->this_idx].entry_pos = (uint32_t)write_pos;
...
int traft_termlog_wait_more(traft_termlog *log, uint32_t last_idx, uint32_t *newest_idx, int max_wait_ms) {
  ...
  gettimeofday(&start);
  traft_rwlock_rdlock(&lock->lock);
  *newest_idx = log->local_committed_idx;
  struct timeval now;
  gettimeofday(&now);
  ...
  pthread_mutex_unlock(&lock->entries_lock);
  return 0;
}
